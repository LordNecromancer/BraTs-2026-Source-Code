...
class nnUNetPredictor(object):
    ...
    def predict_from_files(self,
        ...
        n = min(10, len(case_names))

        self.selected_cases = set(random.sample(case_names, n))

        print("\nSelected images for prompt visualization:\n")

        for c in sorted(self.selected_cases):
            print(c)
    ...
    def predict_from_data_iterator(self,
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if ofile is not None:
                    self.current_case = os.path.basename(ofile)
                else:
                    self.current_case = "Unknown"
        ...
        # Average prompt weights over all sliding-window patches
        for stage in ["stage1", "stage2", "stage3"]:
            for case in self.prompt_logs[stage]:
                self.prompt_logs[stage][case] = np.mean(
                    self.prompt_logs[stage][case],
                    axis=0
                )
        ...
        for stage in ["stage1", "stage2", "stage3"]:
            ...
            with open(csv_name, "w", newline="") as f:
                ...
                writer.writerow(
                    ["Case"] + [f"Prompt{i}" for i in range(10)]
                )

                for case, w in self.prompt_logs[stage].items():
                    writer.writerow([case] + list(w))
        ...
        # -----------------------------
        for stage in stages:
            if len(self.prompt_logs[stage]) == 0:
                matrices.append(None)
                case_lists.append(None)
                continue

            cases = list(self.prompt_logs[stage].keys())

            matrix = np.stack([
                self.prompt_logs[stage][c]
                for c in cases
            ])
        ...
        for ax, stage, matrix, cases in zip(
                axes,
                stages,
                matrices,
                case_lists):
            ...
            ax.set_xticklabels([f"P{i}" for i in range(matrix.shape[1])])

            ax.set_yticks(range(len(cases)))
            ax.set_yticklabels(cases, fontsize=8)

        # Only left subplot gets y-label
        axes[0].set_ylabel("Validation Case")
...
def predict_entry_point():
    ...
    print(f"Average inference: {np.mean(times):.3f} s")
